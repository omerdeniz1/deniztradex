...
-- >>> supabase/migrations/20260918160000_guard_maintenance_bypass.sql
-- NOT: kronolojik olarak en yeni dosya olmasına rağmen burada,
-- wallet_transfer backfill'inden ÖNCE yer alır — çünkü backfill'in
-- çalışması için bu baypasın kurulu olması gerekir. Tek seferlik
-- bundle çalışmasında yürütme sırası = dosya sırasıdır.
-- ============================================================
-- DenizTradeX — bakım (maintenance) baypası: tablo sahibi engellenmez
--
-- Hata: bundle SQL Editor'de çalıştırılırken wallet_transfer
-- backfill'i şu hatayla duruyordu:
--   ERROR P0001: süper admin hesabına müdahale edemezsin
-- Neden: SQL Editor doğrudan `postgres` rolüyle, JWT'siz bağlanır;
--   `auth.uid()` NULL → SÜPER ADMİN satırına değen HER update reddedilir.
-- Çözüm: koruyucu tetikleyici, doğrudan tablo-sahibi bakım
-- oturumlarında satırı aynen geçirir; API rolleri için TÜM kontroller
-- aynen çalışır. KRİTİK: kontrol `session_user` iledir
-- (`current_user` SECURITY DEFINER içinde her zaman sahiptir).
-- Güvenlik kaybı YOK: owner zaten trigger'ları devre dışı bırakabilir.
--
-- Idempotent: tekrar çalıştırılabilir.
-- ============================================================
...
-- >>> supabase/migrations/20260918170000_dnz_token.sql
-- ============================================================
...
-- >>> supabase/migrations/20260918180000_dnz_amm_pool.sql
-- ============================================================
...
-- >>> supabase/migrations/20260918190000_virtual_24h_changes.sql
-- ============================================================
...
-- >>> supabase/migrations/20260918200000_retail_bot_realtime.sql
-- ============================================================
...
-- >>> supabase/migrations/20260918210000_dnz_pool_resize.sql
-- ============================================================
...
-- >>> supabase/migrations/20260923000000_auto_market_maker.sql
-- ============================================================
...
-- >>> supabase/migrations/20260923000001_auto_market_cron.sql
-- ============================================================
...
-- >>> supabase/migrations/20260923000002_forum_profiles.sql
-- ============================================================
...
-- >>> supabase/migrations/20260923000003_transfer_fee_display_name.sql
-- ============================================================
...
-- >>> supabase/migrations/20260923000004_transfer_counterparty.sql
-- ============================================================
...
-- >>> supabase/migrations/20260924000000_forum_bot_likes.sql
-- ============================================================
...
-- >>> supabase/migrations/20260924000001_auto_bot_fee_symmetry.sql
-- ============================================================
...
-- >>> supabase/migrations/20260924000002_kursat_boost.sql
-- ============================================================
...
-- >>> supabase/migrations/20260926000000_bot_trade_fee_symmetry.sql
-- ============================================================
...
-- >>> supabase/migrations/20260926000001_forum_limit_5000.sql
-- ============================================================
...
-- >>> supabase/migrations/20260926000002_forum_likes_sticky.sql
-- ============================================================
...
-- >>> supabase/migrations/20260926000003_bot_volatility_up.sql
-- ============================================================
-- DenizTradeX — Bot oynaklığı büyütme (daha canlı fiyat hareketi)
--
-- Hamle büyüklükleri ~2.5-3x artırılır (tek tikte yıkım yok):
--   - `execute_auto_market_trade` cap: havuzun binde 2'si → binde 4'ü
--     (25.000 USDT mutlak tavan aynen korunur).
--   - `run_auto_market_bot` (pg_cron): tur tutarı rezervin
--     %0.04–0.15'i → %0.1–0.4'ü, cap binde 3 → binde 5.
-- İstemci aralıkları (`autoMarketMakerService` FRACTION_RANGE +
-- perakende 20–300$) ile uyumludur; simetri ve %75 ortalama-dönüş
-- mantığı aynen korunur — yönsüz, sadece daha geniş bant.
--
-- Idempotent: tekrar çalıştırılabilir.
-- ============================================================

-- ------------------------------------------------------------
-- 1) execute_auto_market_trade: cap binde 4
-- ------------------------------------------------------------
create or replace function public.execute_auto_market_trade(
  p_symbol      text,
  p_trade_type  text,
...
declare
  v_fee_rate   constant numeric := 0.003;
  v_cap_frac   constant numeric := 0.004;
  v_cap_abs    constant numeric := 25000;
  v_amount     numeric;
  v_eff        numeric;
  v_rusdt      numeric;
...
-- ------------------------------------------------------------
-- 2) run_auto_market_bot (pg_cron turu): tutar %0.1–0.4, cap binde 5
-- ------------------------------------------------------------
create or replace function public.run_auto_market_bot()
returns jsonb
language plpgsql
...
declare
  v_fee_rate   constant numeric := 0.003;
  v_cap_frac   constant numeric := 0.005;
  v_cap_abs    constant numeric := 25000;
  v_band       constant numeric := 0.08;
  v_enabled    boolean := true;
  v_intensity  text := 'normal';
...
begin
  ...
  for i in 1..v_rounds loop
    ...
    -- Tutar: rezervin %0.1–0.4'ü, cap ile kırpılır.
    v_amount := least(
      v_rusdt * (0.001 + random() * 0.003),
      v_rusdt * v_cap_frac,
      v_cap_abs
    );
    if v_amount is null or v_amount <= 0 then
