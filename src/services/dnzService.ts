 ...
 *
 * - Token ekonomisi: toplam arz 200M (sabit), açılış $0.50.
 * - Fiyat TEK kaynaktan gelir: sanal AMM havuzu (`virtual_coins` DNZ
 *   satırı; istemcide `listVirtualCoins`, sunucuda pool). Ayrı simüle
 *   fiyat yoktur — grafik, etki ve değerlemeler havuz fiyatıyla tutarlıdır.
 * - Sunucu bakiye/defter tutar (`dnz_balances` / `dnz_ledger`); takaslar
 *   `execute_dnz_trade` RPC'sinden (uzak) veya yerel AMM motorundan geçer.
...
/** Toplam arz: 200.000.000 DNZ (sabit, basım yok; açılışta tamamı havuzda). */
export const DNZ_TOTAL_SUPPLY = 200_000_000
/** Açılış/listeleme referans fiyatı (USDT) — havuz tohumuyla birebir (100M/200M). */
